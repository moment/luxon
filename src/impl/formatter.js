...
export class Formatter {
  ...
  formatInstantFromString(inst, fmt) {
    ...
    const formatOffset = (opts) => {
      ...
      const minutes = Math.abs(inst.offset() % 60);
      const sign = hours > 0 ? '+' : '-';
      const formatter = n => this.num(n, opts.format === 'short' ? 2 : 0);
      const base = sign + formatter(Math.abs(hours));

      switch (opts.format) {
      case 'short': return `${sign}${formatter(Math.abs(hours))}:${formatter(minutes)}`;
      case 'narrow':
  ...
  formatDurationFromString(dur, fmt) {
    ...
    const realTokens = tokens.reduce((found, { literal, val }) =>
                                     (literal ? found : found.concat(val)), []);
    const collapsed = dur.shiftTo(...realTokens.map(t => tokenToField(t)));
