...
function unsupportedZone(zone: Zone) {
...
function normalizeUnit(unit: string): keyof GenericDateTime {
  const normalized = {
    year: "year",
    years: "year",
    month: "month",
  ...
  return normalized as keyof GenericDateTime;
}

// this is a dumbed down version of fromObject() that runs about 60% faster
// but doesn't do any validation, makes a bunch of assumptions about what units
// are present, and so on.
function quickDT(obj: GregorianDateTime, opts: DateTimeOptions): DateTime {
...
function diffRelative(start: DateTime, end: DateTime, opts: DiffRelativeOptions) {
  const round = isUndefined(opts.round) ? true : opts.round,
    format = (c: number, unit: Intl.RelativeTimeFormatUnit) => {
      c = roundTo(c, round || opts.calendary ? 0 : 2, true);
      const formatter = Locale.create(end.locale || undefined)
        .clone(opts)
        .relFormatter(opts);
...
function lastOpts(argList: unknown[]): [DateTimeOptions, number[]] {
  ...
  if (argList.length > 0 && typeof argList[argList.length - 1] === "object") {
    opts = argList[argList.length - 1] as DateTimeOptions;
...
export default class DateTime {
  ...
  private readonly c: Readonly<GregorianDateTime> | null;
  private readonly invalid: Invalid | null;
  private readonly isLuxonDateTime: true;
  private readonly loc: Locale;
  private readonly o: number | null;
  private readonly ts: number;
  private weekData: WeekDateTime | null = null;
  ...
  constructor(config: Config) {
    ...
    if (!invalid) {
      const unchanged =
        config.old && config.old.ts === this.ts && config.old.zone && config.old.zone.equals(zone);
   ...
   * @return {DateTime}
   */
  static local(...args: (number | DateTimeOptions)[]): DateTime {
    const [options, values] = lastOpts(args),
      [year, month, day, hour, minute, second, millisecond] = values;
    return quickDT({ year, month, day, hour, minute, second, millisecond }, options);
  }
   ...
   * @return {DateTime}
   */
  static utc(...args: (number | DateTimeOptions)[]): DateTime {
    const [opts, values] = lastOpts(args),
      [year, month, day, hour, minute, second, millisecond] = values;
   ...
   * @return {DateTime}
   */
  static fromObject(obj: GenericDateTime, opts: DateTimeOptions = {}): DateTime {
    obj = obj || {};
    ...
    let units,
      ...
      objNow: GenericDateTime = tsToObj(tsNow, offsetProvis);
    if (useWeekData) {
      units = orderedWeekUnits;
      defaultValues = defaultWeekUnitValues;
      objNow = gregorianToWeek(objNow as GregorianDateTime);
    } else if (containsOrdinal) {
      units = orderedOrdinalUnits;
      defaultValues = defaultOrdinalUnitValues;
      objNow = gregorianToOrdinal(objNow as GregorianDateTime);
    } else {
      units = orderedUnits;
    ...
    const higherOrderInvalid = useWeekData
        ? hasInvalidWeekData(normalized as WeekDateTime)
        : containsOrdinal
        ? hasInvalidOrdinalData(normalized as OrdinalDateTime)
        : hasInvalidGregorianData(normalized as GregorianDateTime),
      invalid = higherOrderInvalid || hasInvalidTimeData(normalized as TimeObject);
  ...
  static fromFormat(text: string, fmt: string, opts: DateTimeWithZoneOptions = {}): DateTime {
    ...
    const { locale = undefined, numberingSystem = undefined } = opts,
      localeToUse = Locale.fromOpts({
        locale,
        numberingSystem,
        defaultToEN: true,
      }),
      [vals, parsedZone] = parseFromTokens(localeToUse, text, fmt);
    return parseDataToDateTime(vals || null, parsedZone || null, opts, `format ${fmt}`, text);
  }

  /**
   * @deprecated use fromFormat instead
   */
  static fromString(text: string, fmt: string, opts: DateTimeWithZoneOptions = {}): DateTime {
   ...
   * @type {string}
   */
  get numberingSystem(): NumberingSystem | null {
   ...
   * @type {string}
   */
  get outputCalendar(): CalendarSystem | null {
   ...
   */
  get year(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).year : NaN;
  }
   ...
   */
  get quarter(): number {
    return this.isValid ? Math.ceil((this.c as Readonly<GregorianDateTime>).month / 3) : NaN;
  }
   ...
   */
  get month(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).month : NaN;
  }
   ...
   */
  get day(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).day : NaN;
  }
   ...
   */
  get hour(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).hour : NaN;
  }
   ...
   */
  get minute(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).minute : NaN;
  }
   ...
   */
  get second(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).second : NaN;
  }
  ...
  get millisecond(): number {
    return this.isValid ? (this.c as Readonly<GregorianDateTime>).millisecond : NaN;
  }
   ...
   */
  get weekYear(): number {
    return this.isValid ? (this.possiblyCachedWeekData() as WeekDateTime).weekYear : NaN;
  }
  ...
  get weekNumber(): number {
    return this.isValid ? (this.possiblyCachedWeekData() as WeekDateTime).weekNumber : NaN;
  }
   ...
   */
  get weekday(): number {
    return this.isValid ? (this.possiblyCachedWeekData() as WeekDateTime).weekday : NaN;
  }
   ...
   */
  get ordinal(): number {
    return this.isValid ? gregorianToOrdinal(this.c as Readonly<GregorianDateTime>).ordinal : NaN;
  }
   ...
   */
  get offset(): number {
    return this.isValid ? +(this.o as number) : NaN;
  }
  ...
  resolvedLocaleOptions(
    opts: LocaleOptions = {}
  ): Partial<Intl.ResolvedDateTimeFormatOptions> & { outputCalendar: string } {
  ...
  setZone(
    zone: ZoneLike,
    { keepLocalTime = false, keepCalendarTime = false }: SetZoneOptions = {}
  ): DateTime {
    ...
    if (zone.equals(this.zone)) {
      ...
      if (keepLocalTime || keepCalendarTime) {
        ...
        const asObj = this.toObject() as GregorianDateTime;
   ...
   * @return {DateTime}
   */
  reconfigure({ locale, numberingSystem, outputCalendar }: LocaleOptions = {}): DateTime {
  ...
  set(values: GenericDateTime): DateTime {
    ...
    if (settingWeekStuff) {
      mixed = weekToGregorian({
        ...gregorianToWeek(this.c as GregorianDateTime),
        ...normalized,
      });
    } else if (!isUndefined(normalized.ordinal)) {
      mixed = ordinalToGregorian({
        ...gregorianToOrdinal(this.c as GregorianDateTime),
        ...normalized,
      });
    } else {
      mixed = { ...(this.toObject() as GregorianDateTime), ...normalized };
    ...
    const [ts, o] = objToTS(mixed, this.o as number, this.zone);
  ...
  plus(duration: DurationLike): DateTime {
    ...
    let adjusted;
    try {
      adjusted = this.adjustTime(dur);
    } catch (e) {
      return new DateTime({ invalid: new Invalid("Invalid Duration") });
    }
    if (adjusted === this) return this;
    return this.clone(adjusted as { ts: number; o: number });
  }
  ...
  */
  ...
  toLocaleParts(opts: Intl.DateTimeFormatOptions | LocaleOptions = {}): Intl.DateTimeFormatPart[] {
    const localeOpts: LocaleOptions = {
      locale: "locale" in opts ? opts.locale : undefined,
      outputCalendar: "outputCalendar" in opts ? opts.outputCalendar : undefined,
      numberingSystem:
        "numberingSystem" in opts ? (opts.numberingSystem as NumberingSystem) : undefined,
    };
    return this.isValid
      ? Formatter.create(this.loc.clone(localeOpts), opts).formatDateTimeParts(this)
      : [];
  }
   ...
   * @return {string}
   */
  toISO(opts: ToTechTimeOptions = {}): string | null {
    if (!this.isValid) {
      return null;
    }
   ...
   * @return {string}
   */
  toISODate({ format = "extended" }: { format?: "basic" | "extended" } = {}): string | null {
  ...
  toSQLTime({
    includeOffset = true,
    includeZone = false,
  }: Pick<ToTechTimeOptions, "includeOffset" | "includeZone"> = {}): string | null {
    return toTechTimeFormat(this, {
      includeOffset,
      includeZone,
      spaceZone: true,
   ...
   * @return {string}
   */
  toSQL(opts: Pick<ToTechTimeOptions, "includeOffset" | "includeZone"> = {}): string | null {
    if (!this.isValid) {
      return null;
    }
   ...
   * @return {Object}
   */
  toObject(opts: { includeConfig?: boolean } = {}): FromDateTimeToObject {
    if (!this.isValid) return {};

    const base: FromDateTimeToObject = { ...this.c };
    ...
    return base as FromDateTimeToObject;
  }
  ...
  diff(
    ...
    const durOpts: DurationOptions = {
      locale: this.locale || undefined,
      numberingSystem: this.numberingSystem || undefined,
      ...opts,
    };
  ...
  until(otherDateTime: DateTime): Interval {
    return this.isValid
      ...
      : Interval.invalid("Invalid DateTime");
  }
  ...
  hasSame(otherDateTime: DateTime, unit: DurationUnit): boolean {
    ...
    return (
      otherZoneDateTime.startOf(unit).valueOf() <= inputMs &&
      inputMs <= otherZoneDateTime.endOf(unit).valueOf()
    );
  }
   ...
   */
  toRelativeCalendar(options: { base?: DateTime } = {}): string | null {
    if (!this.isValid) return null;
  ...
  private adjustTime(dur: Duration) {
    if (!this.isValid) return this;
    const oPre = this.o as number,
      year = (this.c as GregorianDateTime).year + Math.trunc(dur.years),
      month =
        (this.c as GregorianDateTime).month + Math.trunc(dur.months) + Math.trunc(dur.quarters) * 3,
      c = {
        ...this.c,
        year,
        month,
        day:
          Math.min((this.c as GregorianDateTime).day, daysInMonth(year, month)) +
          Math.trunc(dur.days) +
 ...
 * @private
 */
export function friendlyDateTime(dateTimeish: DateTimeLike): DateTime {
  if (DateTime.isDateTime(dateTimeish)) {
    return dateTimeish;
  } else if (dateTimeish && isDate(dateTimeish)) {
