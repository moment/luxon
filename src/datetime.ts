...
import { parseHTTPDate, parseISODate, parseRFC2822Date, parseSQL } from "./impl/regexParser.ts";
import {
  expandMacroTokens,
  explainFromTokens,
  formatOptsToTokens,
...
function quickDT(obj: PartialNullable<DateTimeObject>, opts: DateTimeWithZoneOptions): DateTime {
  ...
  if (!isUndefined(obj.year)) {
    ...
    checkValidGregorianData(obj.year, obj.month, obj.day);
    checkValidTimeData(obj.hour, obj.minute, obj.second, obj.millisecond);
...
export default class DateTime {
  ...
  static fromMillis(milliseconds: number, options: DateTimeWithZoneOptions = {}): DateTime {
    checkIntegerBetween(milliseconds, -MAX_DATE, MAX_DATE, "milliseconds");
    return new DateTime(
      {
        ts: milliseconds,
  ...
  static fromSeconds(seconds: number, options: DateTimeWithZoneOptions = {}): DateTime {
    checkIntegerBetween(seconds, -(MAX_DATE / 1000), MAX_DATE / 1000, "seconds");
    return new DateTime(
      {
        ts: seconds * 1000,
        zone: normalizeZone(options.zone, Settings.defaultZone),
  ...
  static invalid(reason: string | Invalid, explanation: string | null = null): DateTime {
    throw new Error(
      `Invalid DateTime is no longer a thing: ${reason instanceof Invalid ? reason.toMessage() : reason}`
    );
  }
   ...
   * The resulting format will not necessarily be able to parse an output generated by `toLocaleString` with the same
   * set of options, specifically when options are set that do not have a corresponding parsing token.
   * In such cases, the relevant parts in the formatted date will be silently ignored and not be included in the
   * parsing format.
   *
   * The `dateStyle` and `timeStyle` options are not supported.
   *
   * The `timeZoneName` option is handled on a best-effort basis and the resulting parsing format will not match the
   * `toLocaleString` output for the same set of options in almost all cases.
   * @param formatOpts
   * @param localeOpts
   * @returns {string}
   */
  static parseFormatForOpts(
    ...
    checkIntlDtfOptions(formatOpts);
    const tokenList = formatOptsToTokens(formatOpts, Locale.fromObject(localeOpts));
    return formatTokensToFormat(tokenList);
  }

  /**
   * Produce the fully expanded format token for the locale. That is, any macro tokens like tt are expanded according
   * to the locale.
   *
   * @param fmt
   * @param localeOpts
  ...
  static expandFormat(fmt: string, localeOpts: LocaleOptions = {}): string {
    ...
    return formatTokensToFormat(expanded);
  }
  ...
  get isWeekend(): boolean {
    return this.#loc.getWeekendDays().includes(this.weekday);
  }
  ...
  get localWeekday(): number {
    return this.#possiblyCachedLocalWeekData().weekday;
  }
  ...
  get localWeekNumber(): number {
    return this.#possiblyCachedLocalWeekData().weekNumber;
  }
  ...
  get localWeekYear(): number {
    return this.#possiblyCachedLocalWeekData().weekYear;
  }
  ...
  get monthShort(): string {
    return Info.months("short", { locObj: this.#loc })[this.month - 1];
  }
  ...
  get monthLong(): string {
    return Info.months("long", { locObj: this.#loc })[this.month - 1];
  }
  ...
  get weekdayShort(): string {
    return Info.weekdays("short", { locObj: this.#loc })[this.weekday - 1];
  }
  ...
  get weekdayLong(): string {
    return Info.weekdays("long", { locObj: this.#loc })[this.weekday - 1];
  }
  ...
  get isOffsetFixed(): boolean {
    return this.zone.isOffsetFixed();
  }
  ...
  toLocaleString(
    ...
    // TODO: Validate / check locale options
    return Formatter.create(this.#loc.clone(opts), formatOpts).formatDateTime(this);
  }
   ...
   */
  until(otherDateTime: DateTimeLike): Interval {
