...
import { withDTFnoFormatToParts } from "../helpers";
...
test("IANAZone.offsetName with a long format", () => {
  ...
  const offsetName = zone.offsetName(1552089600, {
    format: "long",
    locale: "en-US",
  });
...
test("IANAZone.offset works", () => {
  const ianaZone = new IANAZone("America/New_York");
  expect(ianaZone.offset(1631321986)).toBe(-300);
});

withDTFnoFormatToParts("IANAZone.offset works", () => {
  const ianaZone = new IANAZone("America/New_York");
  expect(ianaZone.offset(1631321986)).toBe(-300);
});
