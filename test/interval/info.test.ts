...
test("Interval#length('days') returns 1 for yesterday", () => {
  expect(atHour(13).minus({ days: 1 }).until(atHour(13)).length("days")).toBe(1);
});
...
test("Interval#toDuration creates a duration in those units", () => {
  const int = Interval.fromDateTimes(atHour(9), atHour(13));

  // @ts-expect-error test
  expect(int.toDuration().toObject()).toEqual({
    milliseconds: 4 * 3600 * 1000,
  });
  expect(int.toDuration("milliseconds")?.toObject()).toEqual({
    milliseconds: 4 * 3600 * 1000,
  });
...
test("Interval#toDuration accepts multiple units", () => {
  const int = Interval.fromDateTimes(
    atHour(9).plus({ minutes: 3 }),
    atHour(13).plus({ minutes: 47 })
  );

  expect(int.toDuration(["hours", "minutes"])?.toObject()).toEqual({
    hours: 4,
    minutes: 44,
  });
});
...
test("Interval#isAfter returns false for fully before the input ", () => {
  const n = DateTime.fromISO("1982-05-25T06:00"),
    i = Interval.fromDateTimes(n.minus({ day: 2 }), n.minus({ days: 1 }));
  expect(i.isAfter(n)).toBe(false);
});
