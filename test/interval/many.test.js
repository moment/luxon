...
const todayFrom = (h1, h2, opts) => Interval.fromInstants(todayAt(h1), todayAt(h2), opts);
...
const diff = (interval, items, expected) => {
...
test('Interval#divideEqually should split a 1m30s into 3 30-second parts', () => {
  ...
  const split = after(todayAt(9), 1, 30).divideEqually(3);
  expect(split.length).toBe(3);
