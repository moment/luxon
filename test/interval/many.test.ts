...
import { DateTimeLike } from "../../src/types/datetime";
import { atHour, withDefaultZone } from "../helpers";

const fromISOs = (s: string, e: string) => DateTime.fromISO(s).until(DateTime.fromISO(e)),
  todayFrom = (h1: number, h2: number) => Interval.fromDateTimes(atHour(h1), atHour(h2));
...
test("Interval#intersection returns the intersection for overlapping intervals", () => {
  ...
  if (!intersection) fail("Intersection returned null");
  expect(intersection.equals(todayFrom(5, 7))).toBeTruthy();
});
...
test("Interval#intersection returns invalid for invalid intervals", () => {
  const intersection = Interval.invalid("any reason").intersection(todayFrom(8, 10));
  if (!intersection) fail("Intersection returned null");
  expect(intersection.isValid).toBeFalsy();
});
...
test("Interval#engulfs", () => {
  ...
  expect(i.engulfs(todayFrom(13, 15))).toBeFalsy();
  expect(i.engulfs(todayFrom(11, 15))).toBeFalsy();
  expect(i.engulfs(todayFrom(6, 8))).toBeFalsy();
  expect(i.engulfs(todayFrom(6, 10))).toBeFalsy();
  expect(i.engulfs(todayFrom(8, 13))).toBeFalsy();

  expect(i.engulfs(todayFrom(10, 11))).toBeTruthy();
  expect(i.engulfs(todayFrom(9, 12))).toBeTruthy();
});
...
test("Interval#splitAt breaks up the interval", () => {
  const split = todayFrom(8, 13).splitAt(atHour(9), atHour(11));
...
test("Interval#splitAt returns [] for invalid intervals", () => {
  const split = Interval.invalid("because").splitAt(atHour(9), atHour(11));
  expect(split).toEqual([]);
});

test("Interval#splitAt ignores times outside the interval", () => {
  const allBefore = todayFrom(8, 13).splitAt(atHour(7));
  expect(allBefore.length).toBe(1);
  expect(allBefore[0]).toEqual(todayFrom(8, 13));

  const allAfter = todayFrom(8, 13).splitAt(atHour(14));
  expect(allAfter.length).toBe(1);
  expect(allAfter[0]).toEqual(todayFrom(8, 13));

  const oneBeforeOneDuring = todayFrom(8, 13).splitAt(atHour(7), atHour(11));
  ...
  const oneAfterOneDuring = todayFrom(8, 13).splitAt(atHour(11), atHour(15));
...
test("Interval#divideEqually should split a 1m30s into 3 30-second parts", () => {
  const after = (i: DateTimeLike, m: number, s: number) =>
      Interval.after(i, Duration.fromObject({ minutes: m, seconds: s })),
    split = after(atHour(9), 1, 30).divideEqually(3);
  expect(split.length).toBe(3);
  expect(split[0].equals(after(atHour(9), 0, 30))).toBeTruthy();
  expect(split[2].equals(after(atHour(9).plus({ minutes: 1 }), 0, 30))).toBeTruthy();
});

test("Interval#divideEqually always gives you the right number of parts", () => {
  const int = Interval.after(atHour(9), { minutes: 7 }),
...
test("Interval#mapEndpoints returns a new Interval with the mapped endpoints", () => {
  ...
  if (!mapped.start || !mapped.end) fail("Interval.start or Interval.end are null");
