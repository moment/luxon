...
test("DateTime#plus renders invalid when out of max. datetime range using seconds", () => {
  const d = DateTime.utc(1970, 1, 1, 0, 0, 0, 0).plus({
    second: 1e8 * 24 * 60 * 60 + 1,
  });
  expect(d.isValid).toBe(false);
});
...
test("DateTime#minus renders invalid when out of max. datetime range using seconds", () => {
  const d = DateTime.utc(1970, 1, 1, 0, 0, 0, 0).minus({
    second: 1e8 * 24 * 60 * 60 + 1,
  });
  expect(d.isValid).toBe(false);
});

test("DateTime#minus renders invalid when out of max. datetime range using IANAZone", () => {
  const d = DateTime.utc(1970, 1, 1, 0, 0, 0, 0)
    .setZone("America/Los_Angeles")
...
test("DateTime#startOf throws on invalid units", () => {
  expect(() =>
    ...
    DateTime.fromISO("2016-03-12T10:00").startOf("splork")
  ).toThrow();
  // @ts-expect-error test
