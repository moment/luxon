...
import { DurationObject, DurationUnit } from "../../src/types/duration";
...
const diffFromObjs = (
  ...
  DateTime.fromObject(o1, { zone: "America/New_York" }).diff(
    DateTime.fromObject(o2, { zone: "America/New_York" }),
    units
  );
const diffObjs = (o1: DurationObject, o2: DurationObject, units?: DurationUnit | DurationUnit[]) =>
...
withNow("DateTime#diffNow defaults to milliseconds", DateTime.local(2017, 5, 15), () => {
  const dt = DateTime.local(2014, 8, 6),
    dur = dt.diffNow();
  expect(dur.milliseconds).toBe(-87523200000);
});

withNow("DateTime#diffNow accepts units", DateTime.local(2017, 5, 15), () => {
  const dt = DateTime.local(2014, 8, 6),
    dur = dt.diffNow("days");
  expect(dur.days).toBe(-1013);
});

withNow("DateTime#diffNow passes through options", DateTime.local(2017, 5, 15), () => {
  const dt = DateTime.local(2014, 8, 6),
