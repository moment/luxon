...
import { withDefaultLocale, withDTFnoFormatToParts } from "../helpers";
...
test("DateTime.fromFormat() throws if you specify meridiem with 24-hour time", () => {
  try {
    DateTime.fromFormat("930PM", "Hmma");
    fail("Did not throw an error");
  } catch (e) {
    expect((e as ConflictingSpecificationError).name).toBe("ConflictingSpecificationError");
  }
});
...
test("DateTime.fromFormat() returns invalid for invalid token", () => {
  expect(DateTime.fromFormat("2021-09-08", "w").isValid).toBeFalsy();
});
    ...
    for (const locale of [undefined, "en-gb", "de"]) {
      for (const format of formats) {
        ...
        if (expectInvalid) {
          ...
          if (!expectEqual) fail();
          for (const _key of Object.keys(expectEqual)) {
            const key = _key as keyof DateTime;
...
withDTFnoFormatToParts("DateTime.fromFormat() still works", () => {
  const i = DateTime.fromFormat("1982/05/25 09:10:11.445", "yyyy/MM/dd HH:mm:ss.SSS");
  expect(i.year).toBe(1982);
  expect(i.month).toBe(5);
  expect(i.day).toBe(25);
...
function keyCount(o: object | undefined | null) {
...
test("DateTime.fromFormatExplain() takes the same options as fromFormat", () => {
  const ex = DateTime.fromFormatExplain("Janv. 25 1982", "LLL dd yyyy", {
    locale: "fr",
  });
  expect(keyCount(ex.result)).toBe(3);
});

withDTFnoFormatToParts("DateTime.fromFormatExplain() works with meridiems", () => {
  expect(() => DateTime.fromFormatExplain("AM", "a")).not.toThrow();
});
