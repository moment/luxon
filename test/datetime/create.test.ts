...
test("DateTime.local accepts an options hash in any position", () => {
  const options = {
    zone: "Europe/Paris",
    numberingSystem: "beng" as NumberingSystem,
    outputCalendar: "islamic" as CalendarSystem,
    locale: "fr",
  };
  const args = [
...
test("DateTime.fromObject validates weekdays", () => {
  let dt = DateTime.fromObject({ year: 2005, month: 12, day: 13, weekday: 1 });
  expect(dt.isValid).toBe(false);

  dt = DateTime.fromObject({ year: 2005, month: 12, day: 13, weekday: 2 });
  expect(dt.isValid).toBe(true);
});
