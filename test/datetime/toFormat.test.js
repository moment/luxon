...
test("DateTime#toFormat('f') returns a short date/time representation without seconds", () => {
  ...
  expect(dt.reconfigure({ locale: "fr" }).toFormat("f")).toBeOneOf([
    "25/05/1982 09:23",
    // Old format for WebKit
    "25/05/1982, 09:23",
  ]);
  expect(dt.set({ hour: 13 }).reconfigure({ locale: "fr" }).toFormat("f")).toBeOneOf([
    "25/05/1982 13:23",
    "25/05/1982, 13:23",
  ]);
});
...
test("DateTime#toFormat('fff') returns a medium date/time representation without seconds", () => {
  // WebKit has old ICU and returns a comma

  expect(ny.toFormat("fff")).toBeOneOf([
    "May 25, 1982 at 9:23 AM EDT",
    "May 25, 1982, 9:23 AM EDT",
  ]);
  expect(ny.set({ hour: 13 }).toFormat("fff")).toBeOneOf([
    "May 25, 1982 at 1:23 PM EDT",
    "May 25, 1982, 1:23 PM EDT",
  ]);
  expect(ny.set({ month: 8 }).toFormat("fff")).toBeOneOf([
    "August 25, 1982 at 9:23 AM EDT",
    "August 25, 1982, 9:23 AM EDT",
  ]);
  expect(ny.reconfigure({ locale: "fr" }).toFormat("fff")).toBeOneOf([
    "25 mai 1982 à 09:23 UTC−4",
    "25 mai 1982, 09:23 UTC−4",
  ]);
...
test("DateTime#toFormat('ffff') returns a long date/time representation without seconds", () => {
  // WebKit has old ICU and returns a comma and old name for EDT in French

  expect(ny.toFormat("ffff")).toBeOneOf([
    "Tuesday, May 25, 1982 at 9:23 AM Eastern Daylight Time",
    "Tuesday, May 25, 1982, 9:23 AM Eastern Daylight Time",
  ]);
  ...
  expect(ny.set({ hour: 13 }).reconfigure({ locale: "fr" }).toFormat("ffff")).toBeOneOf([
...
test("DateTime#toFormat('F') returns a short date/time representation with seconds", () => {
  ...
  // Safari has old ICU and has a comma here
  expect(dt.reconfigure({ locale: "fr" }).toFormat("F")).toBeOneOf([
    "25/05/1982 09:23:54",
    "25/05/1982, 09:23:54",
  ]);
...
test("DateTime#toFormat('FFFF') returns a long date/time representation without seconds", () => {
  ...
  expect(ny.toFormat("FFFF")).toBeOneOf([
    ...
    "Tuesday, May 25, 1982, 9:23:54 AM Eastern Daylight Time",
  ]);
  expect(ny.set({ hour: 13 }).toFormat("FFFF")).toBeOneOf([
