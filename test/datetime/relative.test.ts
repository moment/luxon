...
test("DateTime#toRelative takes a unit argument", () => {
  ...
  expect(
    base
      ...
      .toRelative({ base, unit: ["days", "hours", "minutes"] })
  ).toBe("0 minutes ago");
...
withoutRTF("DateTime#toRelative works without RTF", () => {
...
withoutRTF("DateTime#toRelative falls back to English", () => {
  const base = DateTime.fromObject({ year: 2019, month: 12, day: 25 });
...
withoutRTF("DateTime#toRelativeCalendar works without RTF", () => {
  const base = DateTime.fromObject({ year: 2019, month: 10, day: 25 });
  expect(base.plus({ months: 1 }).toRelativeCalendar({ base })).toBe("next month");
});

withoutRTF("DateTime#toRelativeCalendar falls back to English", () => {
  const base = DateTime.fromObject({ year: 2019, month: 12, day: 25 });
