...
import { DateTime, Info } from "../../src/luxon.ts";
...
test.skipIf(isMissingLocaleWeekInfo)(
  "startOf(week) with useLocaleWeeks adheres to the locale",
  () => {
...
//------
test.skipIf(isMissingLocaleWeekInfo)(
  "endOf(week) with useLocaleWeeks adheres to the locale",
  () => {
    const dt = DateTime.fromISO("2023-06-14T13:00:00Z", { setZone: true });
    expect(
      dt.reconfigure({ locale: "de-DE" }).endOf("week", { useLocaleWeeks: true }).toISO()
    ).toBe("2023-06-18T23:59:59.999Z");
    expect(
      dt.reconfigure({ locale: "en-US" }).endOf("week", { useLocaleWeeks: true }).toISO()
    ).toBe("2023-06-17T23:59:59.999Z");
  }
);
...
//------
test.skipIf(isMissingLocaleWeekInfo)(
  "hasSame(week) with useLocaleWeeks adheres to the locale",
  () => {
...
//------
describe.skipIf(isMissingLocaleWeekInfo)("localWeekNumber in locale de-DE", () => {
...
describe.skipIf(isMissingLocaleWeekInfo)("localWeekNumber in locale en-US", () => {
...
// .localWeekday
//------
describe.skipIf(isMissingLocaleWeekInfo)("localWeekday in locale en-US", () => {
...
describe.skipIf(isMissingLocaleWeekInfo)("localWeekday in locale de-DE", () => {
...
describe("weeksInLocalWeekYear", () => {
  test.skipIf(isMissingLocaleWeekInfo)("2018 should have 53 weeks in en-US", () => {
    expect(DateTime.local(2018, 6, 1, { locale: "en-US" }).weeksInLocalWeekYear).toBe(52);
  });
  test.skipIf(isMissingLocaleWeekInfo)("2022 should have 53 weeks in en-US", () => {
    expect(DateTime.local(2022, 6, 1, { locale: "en-US" }).weeksInLocalWeekYear).toBe(
      supportsMinDaysInFirstWeek() ? 53 : 52
    );
  });
  test.skipIf(isMissingLocaleWeekInfo)("2022 should have 52 weeks in de-DE", () => {
    expect(DateTime.local(2022, 6, 1, { locale: "de-DE" }).weeksInLocalWeekYear).toBe(52);
  });
  test.skipIf(isMissingLocaleWeekInfo)("2020 should have 53 weeks in de-DE", () => {
