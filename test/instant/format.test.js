...
}); // test("Instant#toFormatString('GG') returns the full era", t => { //  let i = inst(); //  t.is(i.toFormatString('GG'), 'Anno Domini'); //  t.is(i.year(-21).toFormatString('GG'), 'Before Christ'); //  t.end(); // }); // test("Instant#toFormatString('GGGGG') returns the narrow era", t => { //  let i = inst(); //  t.is(i.toFormatString('GGGG'), 'A'); //  t.is(i.year(-21).toFormatString('GGGGG'), 'B'); //  t.end(); // });
...
}); // numbering is disabled while we're still using the polyfill for number formatting // test('Instant#numbering() overides the numbering system from the locale', t => { //  let i = inst(); //  t.is(i.numbering('beng').toFormatString('S'), '১২৩'); //  t.end();
// });
